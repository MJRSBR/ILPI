...
def calcular_brief_mpi(df, keep_raw=False):
    ...
    # ABVD
    basic = _safe_col(df, "basic_activities_diffic").map({1: 1, 2: 0}).fillna(0).astype(int)
    ...
    difficulties = _safe_col(df, "elder_difficulties").map({1: 0, 2: 1, 3: 0}).fillna(0)
...
df_score
# %%
# Agrupa por institution_name e Risco por instituíção
general_score = df_score.groupby(['risk']).size().reset_index(name='total')
general_score["prop"] = round(general_score['total'] / general_score["total"].sum(), 2)
general_score
# %%
salvar_tabela_como_imagem(
    general_score,
    'Tabela_resumo_geral_score_fragilidade_prop.png',
    titulo='Score de Fragilidade dos Residentes',
    largura_total_max= 100,
)
# %%
# %%
# Agrupa por institution_name e Risco por instituíção
ilpi_score = df_score.groupby(['id_institution', 'risk']).size().reset_index(name='total')
ilpi_score["prop"] = round(ilpi_score['total'] / ilpi_score["total"].sum(), 2)
ilpi_score
# %%
salvar_tabela_como_imagem(
    ilpi_score,
    'Tabela_resumo_score_fragilidade_prop.png',
